...
import { ClearChat, Communities, SendCommunityMessage } from "../../services/API functions/CommunityChatApi";
        ...
        try {
            if (Message || sendImages || sendVideos) {
                ...
                if (Message && Message?.Message?.[0].text?.trim() != '') {
                    const res = await SendCommunityMessage(Message, sendMessageId)
                    socket.emit('CommunityChat', true); MessagesFrt(true)
                    console.log(res, 'ressresres'); setisLoading(false); setIsImageSelected(false);
                }
                if (sendImages) {
                    setisLoading(true)
                    ...
                    const res = await SendCommunityMessage(newMessage, sendMessageId)
                    socket.emit('CommunityChat', true)
                    console.log(res, 'image'); setisLoading(false); setIsImageSelected(false);
                }
                if (sendVideos) {
                    setisLoading(true)
                    ...
                    const res = await SendCommunityMessage(newMessage, sendMessageId)
                    socket.emit('CommunityChat', true)
                    console.log(res, 'video viddeo cvideo'); setisLoading(false); setIsImageSelected(false);
                }
            }
        ...
        <>
            ...
            <div className="flex-1  pt-1   pr-1 ">
                <div className="flex flex-col h-screen  rounded-lg shadow relative  " >
                    ...
                    <div ref={chatContainerRef} className="flex-1 p-4 overflow-y-auto scroll-m overflow-auto snap-end bg-white" key=''>
                            ...
                            <form onSubmit={SendMessage} className="flex justify-center items-start">
                                ...
                                <div className="modal-content p-0 sm:p-4 top-10 rounded-lg relative">
                                    ...
                                    <div className="flex items-center mt-4 pt-2">
                                        ...
                                        />
                                        <button onClick={() => { setSendMessageId(selectedChat?._id), stopVideo() }}
                                            type="submit"
                            ...
                            <form onSubmit={SendMessage}
                                ...
                                <div className="    group">
                                    ...
                                    <a
                                        ...
                                        className="flex cursor-pointer  items-center justify-center text-white bg-blue-700 rounded-full w-7 h-7 hover:bg-blue-800 dark:bg-blue-600 dark:hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 focus:outline-none dark:focus:ring-blue-800"
                                    >
                                        <svg
                                            className={`w-4 h-4 transition-transform  ${isMenuOpen ? "rotate-45" : ""
                                                }`}
                                            aria-hidden="true"
                                            xmlns="http://www.w3.org/2000/svg"
                                            fill="none"
