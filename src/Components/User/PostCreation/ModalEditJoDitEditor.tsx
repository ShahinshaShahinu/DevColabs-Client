...
interface RichTextEditorProps {
    initialValue: string;
    ModalEditorVAue: (newValue: string) => void;
  }




function ModalEditJoDitEditor({ initialValue, ModalEditorVAue  }: RichTextEditorProps) {
    ...
    useEffect(() => {
      if (editorInstance) {
        editorInstance.events.on('change', (newContent: string) => {
          ModalEditorVAue(newContent);
        });
        
      }
      
    }, [editorInstance, ModalEditorVAue]);
  ...
  return (
   <>
    <div style={{ maxWidth: '1100px', margin: 'auto', border: '1px solid #ccc', borderRadius: '10px', height: '500px', overflowY: 'auto' }}>
...
export default ModalEditJoDitEditor